...
void main() {
    ...
    // Grid with multiple scales for infinite look
    outColor = (grid(fragPos3D, 10, true) + grid(fragPos3D, 1, true)) * float(t > 0);
    outColor.a *= fading;
    
    // Make background completely white where no grid
    if (outColor.a < 0.01) {
        outColor = vec4(1.0, 1.0, 1.0, 1.0); // Pure white background
    } else {
        // Blend grid with white background
        outColor.rgb = mix(vec3(1.0, 1.0, 1.0), outColor.rgb, outColor.a * 0.3);
        outColor.a = 1.0;
    }
}